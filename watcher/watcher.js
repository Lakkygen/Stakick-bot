...
const WATCHER_SECRET = String(
  process.env.WATCHER_SECRET || ''
).trim();

const MAX_PAGES = Math.min(
  Math.max(Number(process.env.MAX_PAGES || 3), 1),
  5
);
...
async function ensureBrowser() {
  ...
  startupPromise = (async () => {
    ...
    log(`Profile directory: ${PROFILE_PATH}`);

    /*
     * IMPORTANT:
     * No executablePath is specified here.
     *
     * Playwright will resolve the Chromium installation
     * from PLAYWRIGHT_BROWSERS_PATH=0.
     */
 ...
 *
 * This endpoint deliberately does NOT require the watcher secret
 * so Abasthan can check the service.
 */
...
const server = app.listen(
  ...
  async () => {
    log(
      `Stakick watcher listening on ${PORT}`
    );
    ...
    try {
       ...
       *
       * This allows /health to remain available and
       * expose the exact Chromium startup error.
       */
      errorLog(
        'Browser startup failed:',
        error?.stack ||
